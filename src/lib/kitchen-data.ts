/**
 * Domain model + mock dataset for the workspace.
 *
 ...
 * the app this replaces. Everything here is static; swapping in a real data
 * source means reimplementing the accessors at the bottom of this file and
 * nothing else.
 */
...
/* ---- accessors ------------------------------------------------------ */
/* Swap these for real queries and nothing above the data layer changes.  */

export function getFolder(id: string): Folder | undefined {
  return FOLDERS.find((f) => f.id === id);
}

export function getFolderItems(folderId: string): FolderItem[] {
  const folder = getFolder(folderId);
  if (!folder) return [];
  // Preserve the folder's own ordering rather than the array's.
  return folder.itemIds
    .map((id) => FOLDER_ITEMS.find((i) => i.id === id))
    .filter((i): i is FolderItem => Boolean(i));
}
...
export function getConversationsInFolder(folderId: string): Conversation[] {
...
export function getClients(): Person[] {
  return Object.values(PEOPLE).filter((p) => p.kind === "client");
}
...
export function getDocument(id: string): KDocument | undefined {
  return DOCUMENTS.find((d) => d.id === id);
}

export function getEmbed(id: string): Embed | undefined {
  return EMBEDS.find((e) => e.id === id);
}
...
export function getLibraryFiles(): Array<{
  id: string;
  name: string;
  label: string;
  bytes: number;
  ...
  const uploaded = FOLDER_ITEMS.filter(
    (i) => i.kind === "file" && i.meta.type === "file",
  ).map((i) => {
    ...
    return {
      ...
      authorId: i.authorId,
      folderId: i.folderId,
      source: getFolder(i.folderId)?.name ?? "—",
    };
  });

  const attached = CONVERSATIONS.flatMap((conv) =>
    getConversationFiles(conv.id).map((f) => ({
      id: f.id,
      name: f.name,
      label: f.label,
      bytes: f.bytes,
...
export function getConversationFiles(conversationId: string): Array<
  Attachment & { messageId: string; createdAt: string; authorId: string }
> {
  return getMessages(conversationId).flatMap((m) =>
    (m.attachments ?? []).map((a) => ({
      ...a,
      messageId: m.id,
